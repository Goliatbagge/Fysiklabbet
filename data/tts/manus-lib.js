// ============================================================
// TTS-manus: gemensam modul för uppläsningstext (talsyntes)
//
// Del 1: LaTeX → svensk uppläsningstext
// Del 2: löptext-normalisering (enheter, index, ekvationsnummer)
// Del 3: meningsuppdelning
// Del 4: segmentextraktion ur renderad DOM (browser)
//
// Används av:
//   - data/tts/export-manus.html  (bygger manus-JSON för alla teoriavsnitt)
//   - data/tts/build-manus.js     (Node: nyhetsmanus + orkestrering)
//   - tts.js                      (klientspelaren: matchar segment ↔ element)
//
// Ren logik (del 1–3) är fri från DOM-beroenden och kan testas i Node.
// OBS: ändras något här måste ljudet genereras om:
//   node data/tts/build-manus.js && python data/tts/generate-audio.py
// ============================================================
...
(function (root) {
    ...
    function renderToks(toks, ctx) {
            ...
            if (tok.t === 'cmd') {
                ...
                if (c === 'frac' || c === 'dfrac' || c === 'tfrac') {
                    ...
                    if (c === 'begin' || c === 'end') {
                        ...
                        // \begin{array}{l} — släng även kolumnspecen {l}/{cc}
                        if (c === 'begin' && /^(array|matrix|cases)/.test(env)) {
                            const at = peekIs('grp');
                            if (at >= 0 && /^[lcr|]+$/.test(rawText(toks[at].v))) { i = at + 1; }
                        }
                    }
                } else if (c === '%') {
                    out.push('procent');
        ...
        return out.join(' ')
            .replace(/ /g, ' ')            // NBSP → vanligt mellanslag
            .replace(/\s+/g, ' ')
            .replace(/\s+([,.])/g, '$1')
            .trim();
    }
    ...
    function expandPlainText(text) {
        let s = String(text).replace(/ /g, ' ');   // NBSP → mellanslag
    ...
    function splitSentences(text) {
        let s = String(text);
        // Skydda förkortningarnas punkter med kontrolltecken-sentineler
        // (ALDRIG siffror/bokstäver — de krockar med den riktiga texten)
    ...
    // Talad text för ett blockelement: KaTeX ersätts via TeX-annotationen,
    // kvarvarande <sup> läses som exponent, svg:er tas bort.
    ...
    // Bygg segmentlistan [{ el, text, isFormula }] ur en renderad artikel.
    // Samma vandring körs både vid manus-exporten (byggtid) och i
    // klientspelaren (körtid) — texterna matchas sedan mot varandra för
    // att koppla ljudtidsstämplar till element.
    function extractSegments(container, opts) {
        const items = [];
        const blocks = container.querySelectorAll('h1, h2, h3, h4, p, li, .lab-block-title, .article-fact-title');
        blocks.forEach((el) => {
            ...
            const text = getSpeechText(el, opts);
            if (!text) return;
    ...
    const api = {
        latexToSpeech, expandPlainText, splitSentences,
        getSpeechText, extractSegments,
        NAMN, ENHET_TEXT,
    };
