...
description: Generate AI images using Google's Gemini API ("nano banana"). Use when the user wants to create images, generate pictures, make AI art, edit images, or transform photos. Supports both text-to-image and image-to-image generation.
---

# Gemini Image Generation

Generate high-quality AI images using Google's Gemini image model
("nano banana") directly via the Gemini API (free tier from Google AI Studio).
...
2. **API key** is set (free key from Google AI Studio) — put it in a
...
| `--model` | `-m` | No | Model ID (default: `gemini-3-pro-image`, highest quality; cheaper: `gemini-2.5-flash-image`) |
...
- Put `GEMINI_API_KEY=...` in the project's `.env` file (free key from
...
- Try rephrasing the prompt to be more visual/descriptive (e.g. start with
  "Skapa en bild av …")
- If the model ID is unavailable on the free tier, try
